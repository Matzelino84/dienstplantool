...
export default function DienstplanPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getTeam(), getZuweisungen(monatKey), getDienstplan(monatKey)])
      .then(([team, zuweisungen, plan]) => {
        ...
        // Build plan data from zuweisungen
        ...
        for (const z of zuweisungen) {
          ...
          // Check if this typ already exists for this day (split shift)
  ...
  }, [viewMode, scrollTarget, loading]);

  // If no plan from PocketBase, show demo data as fallback
  useEffect(() => {
    if (!loading && !hasPlan) {
      // Generate simple demo data so the page isn't empty
      getTeam().then((team) => {
        const data: Record<number, DayPlan> = {};
        for (let day = 1; day <= daysInMonth; day++) {
          const seed = day * 13 + 7;
          const shuffled = [...team].sort((a, b) => {
            const sa = (a.vorname.charCodeAt(0) * day + seed) % 100;
            const sb = (b.vorname.charCodeAt(0) * day + seed) % 100;
            return sa - sb;
          });
          if (shuffled.length < 4) continue;
          const dow = new Date(year, month, day).getDay();
          const plan: DayPlan = {
            tagdienst: { name: shuffled[0].vorname, von: "07:00", bis: "19:00", farbe: shuffled[0].farbe || "#666" },
            bd_tag: { name: shuffled[1].vorname, von: "07:00", bis: "19:00", farbe: shuffled[1].farbe || "#666" },
            bd_nacht: { name: shuffled[2].vorname, von: "19:00", bis: "07:00", farbe: shuffled[2].farbe || "#666" },
            nachtdienst: { name: shuffled[3].vorname, von: "19:00", bis: "07:00", farbe: shuffled[3].farbe || "#666" },
          };
          if ((dow === 2 || dow === 5) && shuffled.length >= 5) {
            plan.anmeldung = { name: shuffled[4].vorname, von: "09:00", bis: "14:00", farbe: shuffled[4].farbe || "#666" };
          }
          data[day] = plan;
        }
        setPlanData(data);
  ...
  }, [loading, hasPlan, daysInMonth, year, month]);
      ...
      {loading ? (
        ...
        <div className="flex-1 flex items-center justify-center px-8">
          <div className="text-center">
            ...
            <p className="text-white/40">Noch kein Dienstplan fur diesen Monat</p>
            <p className="text-sm text-white/25 mt-1">Der Admin muss zuerst den Plan generieren</p>
          </div>
        </div>
      ) : (
        <div
                        ...
                        >
                          ...
                          {hasShift ? (
                            <div className="flex flex-col items-center gap-0.5">
                              {shifts.map((s, si) => {
                                const Icon = s.icon;
                                return <Icon key={si} className={cn("h-5 w-5", s.farbe.split(" ")[1] || "text-white/50")} />;
                              })}
                            </div>
                          ) : <div />}
                          {hasShift ? (
                ...
                return (
                  ...
                  <GlassCard className={cn("p-4", isWeekend && "opacity-80")}>
                    ...
                    <div className="space-y-1.5">
                      {grouped.length === 0 && (
                        <p className="text-xs text-white/20 text-center py-2">Kein Plan</p>
                      )}
                      {grouped.map((group) => (
                          ...
                          {group.items.map(({ typ, entry }, gi) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {gi === 0 ? (
                                  <span className="text-xs opacity-50 w-16 shrink-0">{SCHICHT_NAMEN[typ] || typ}</span>
                                ) : (
